...
const walletServicePath = path.join(repoRoot, 'src', 'services', 'walletService.js');
const appHeaderPath = path.join(repoRoot, 'src', 'components', 'layout', 'AppHeader.vue');
...
fs.writeFileSync(walletServicePath, res);

// AppHeader.vue changes
let header = fs.readFileSync(appHeaderPath, 'utf8');
...
fs.writeFileSync(appHeaderPath, header);
