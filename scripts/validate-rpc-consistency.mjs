...
const failOnRetiredDocs = process.env.FAIL_ON_RETIRED_DOCS === "1";
...
const backendApiConfigCandidates = [
  path.resolve(backendRoot, "neo3fura_http/config.yml"),
  path.resolve(backendRoot, "neo3fura_http/config.yml.example"),
  path.resolve(backendRoot, "neo3fura_http/config/config.go"),
  path.resolve(backendRoot, ".worktrees/selfhosted-deploy/neo3fura_http/config/config.go"),
  path.resolve(backendRoot, ".worktrees/reliability-hardening/neo3fura_http/config/config.go"),
];
...
export function collectDocsMissingInBackend({
  docsMethods,
  backendAllowedApis,
  passthroughMethods = new Set(),
  reservedDocMethods = new Set(),
}) {
  const docs = Array.isArray(docsMethods) ? docsMethods : sorted(docsMethods ?? []);
  const backendAllowed = backendAllowedApis instanceof Set ? backendAllowedApis : new Set(backendAllowedApis ?? []);
  const passthrough = passthroughMethods instanceof Set ? passthroughMethods : new Set(passthroughMethods ?? []);
  const reserved = reservedDocMethods instanceof Set ? reservedDocMethods : new Set(reservedDocMethods ?? []);

  return docs.filter(
    (method) => !backendAllowed.has(method) && !passthrough.has(method) && !reserved.has(method)
  );
}
...
export function extractBackendAllowedApis(source) {
  ...
  const yamlRealized = extractYamlRealizedMethods(source);
  if (yamlRealized.size > 0) return yamlRealized;
...
export function extractYamlRealizedMethods(source) {
  const methods = new Set();
  let inMethods = false;
  let inRealized = false;

  for (const line of String(source).split("\n")) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith("#")) continue;

    if (/^methods:\s*$/.test(trimmed)) {
      inMethods = true;
      inRealized = false;
      continue;
    }
    ...
    if (/^[A-Za-z0-9_-]+:\s*$/.test(trimmed) && !/^realized:\s*$/.test(trimmed)) {
    ...
    if (/^realized:\s*$/.test(trimmed)) {
      inRealized = true;
      continue;
    }

    if (!inRealized) continue;

    const item = /^-\s*([A-Za-z0-9_]+)\s*$/.exec(trimmed);
    if (item) {
      methods.add(item[1]);
      continue;
    }
...
async function main() {
  ...
  for (const candidate of backendApiConfigCandidates) {
    if (await exists(candidate)) {
      backendApiConfig = candidate;
      break;
    }
  }
  ...
  const reservedDocsMethods = new Set();
  ...
  for (const method of frontendSorted) {
    ...
    if (isReservedDocContent(docContent)) {
      ...
      reservedDocsMethods.add(method);
    }
  ...
  for (const method of docsSorted) {
    const docFile = methodToDocFileName(method);
    const docPath = path.join(backendDocsDir, docFile);
    if (!(await exists(docPath))) continue;
    const docContent = await fs.readFile(docPath, "utf8");
  ...
  const docsMissingInBackend = collectDocsMissingInBackend({
    docsMethods: docsSorted,
    backendAllowedApis,
    passthroughMethods,
    reservedDocMethods: reservedDocsMethods,
  });
  ...
  if (docsMissingInBackend.length > 0) {
    const message =
      "\nValidation warning: legacy docs reference methods missing from the current backend API allowlist.";
    if (failOnRetiredDocs) {
      console.error(message);
      process.exit(1);
    }
    console.warn(message);
