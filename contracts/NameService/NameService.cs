...
{
    [ManifestExtra("Author", "The R3E Network")]
    [ManifestExtra("Email", "jimmy@r3e.network")]
    [ManifestExtra("Description", "Matrix Name Service")]
    [SupportedStandards("NEP-11")]
    [ContractPermission("*", "onNEP11Payment")]
    [ContractSourceCode("https://github.com/r3e-network/neo-explorer-ui/contracts/NameService")]
    public sealed class NameService : Framework.SmartContract
    {
        ...
        [Safe]
        public static string Symbol() => "MNS";
        ...
        {
            _ = root;
            throw new InvalidOperationException("Only the .matrix root is supported.");
        }
        ...
        {
            var committees = NEO.GetCommittee();
        ...
        private static NameState GetNameState(StorageMap nameMap, ByteString tokenKey)
        {
            ByteString tokenBytes = nameMap[tokenKey];
            if (tokenBytes is null) throw new InvalidOperationException("The name does not exist.");
            return (NameState)StdLib.Deserialize(tokenBytes);
        }

        private static void EnsureMatrixRoot(string[] fragments)
        {
            if (fragments[^1] != MatrixRoot)
                throw new FormatException("Only .matrix names are supported.");
        }

        private static string GetTokenId(string name, string[] fragments)
        {
