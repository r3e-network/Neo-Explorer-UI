...
- `FAIL_ON_RETIRED_DOCS=1` (treat archived legacy Markdown docs that are not in the current backend allowlist as failures)
...
The backend API allowlist is read from `neo3fura_http/config.yml` or `neo3fura_http/config.yml.example`
(`methods.realized`) when present, with the older Go allowlist format kept as a fallback.
...
- catalog methods that are missing current backend support

It warns, but does not fail by default, when archived legacy Markdown docs describe retired methods that are no
longer in the current allowlist.
...
- [@r3e/neo-js-sdk](https://github.com/r3e-network/neo-js-sdk) - Neo N3 JavaScript SDK
