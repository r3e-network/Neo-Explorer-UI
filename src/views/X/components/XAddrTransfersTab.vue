            ...
            <tr v-for="(transfer, i) in items" :key="transferKey(transfer, i)" class="list-row group">
              <td class="table-cell">
                <div class="flex items-center gap-2">
                  ...
                  <XHashLink
                    type="token"
                    :hash="tokenHash(transfer)"
                    :label="transfer.token?.symbol || transfer.token?.name || ''"
                  />
                </div>
              </td>
...
const { items, loading, loadingMore, error, hasMore, loadMore, refresh } = useCursorList((cursor, ctx) =>
  accountService.getTokenTransfers(props.address, { net: getNeoxNet(), cursor, signal: ctx.signal })
);
...
const tokenHash = (transfer) => transfer.token?.address_hash || transfer.token?.address || "";
...
function amountOf(transfer) {
  ...
  if (total && total.value !== undefined && total.value !== null) {
    const decimals = Number(total.decimals ?? transfer.token?.decimals ?? 0);
    return formatUnits(total.value, Number.isFinite(decimals) ? decimals : 0);
  }
