<template>
  <div class="blocks-page">
    <section class="mx-auto max-w-[1400px] px-4 py-6 md:py-8">
      <!-- Breadcrumb -->
      <Breadcrumb
        :items="[{ label: 'Home', to: '/homepage' }, { label: 'Blocks' }]"
      />
      ...
      <div class="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-2">
        <div class="etherscan-card flex items-center gap-3 p-4">
          ...
          <div class="min-w-0">
            ...
            <p
              class="truncate text-sm font-semibold text-text-primary dark:text-gray-100"
            >
      ...
      <div class="etherscan-card overflow-hidden">
        <div class="card-header">
          <p class="text-sm text-text-secondary dark:text-gray-300">
            Block #{{ formatNumber(rangeStart) }} to #{{
              formatNumber(rangeEnd)
            }}
            <span class="hidden sm:inline"
              >(Total of {{ formatNumber(total) }} blocks)</span
            >
          </p>
          <button
...
async function loadStats(forceRefresh = false) {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production")
      console.warn("Failed to load block stats:", err);
  } finally {
