<template>
  <article class="etherscan-card overflow-hidden">
    <header class="card-header">
      <h2 class="text-high text-base font-semibold">{{ $t("homePage.recentTxs") }}</h2>
      <router-link to="/transactions/1" class="btn-outline text-xs">{{ $t("homePage.viewAll") }}</router-link>
    </header>
