    ...
    <section class="page-container py-6 md:py-8">
      ...
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h1 class="page-title neon-glow-text">{{ $t('tools.broadcastMessage.pageTitle') }}</h1>
      ...
      <div class="etherscan-card p-6 md:p-8">
        <div class="max-w-3xl mx-auto space-y-6">
          <div
            class="p-4 rounded-xl bg-indigo-50/80 backdrop-blur-sm border border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-800/50 text-indigo-800 dark:text-indigo-300"
          >
...
async function broadcastMessage() {
  ...
  try {
    ...
    const result = await walletService.invoke({
      ...
      signers: [{ account: fromAddress, scopes: 1 }],
    });
