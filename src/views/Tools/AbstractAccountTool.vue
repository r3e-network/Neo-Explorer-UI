    ...
    <section class="page-container py-6 md:py-8">
      ...
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-start gap-4">
          ...
          <div>
            <h1 class="page-title neon-glow-text">{{ $t("tools.abstractAccount.title") }}</h1>
        ...
        <div class="lg:col-span-4 space-y-4">
          ...
          <div class="etherscan-card p-5 bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-900/10 dark:to-blue-900/10 border-indigo-200/60 dark:border-indigo-900/40">
            ...
            <ol class="space-y-3 text-sm text-mid list-decimal list-inside ml-1">
              <li><strong>{{ $t('tools.abstractAccount.featureZeroDeploy') }}</strong> {{ $t('tools.abstractAccount.featureZeroDeployDesc') }}</li>
              <li><strong>{{ $t('tools.abstractAccount.featureMultiSig') }}</strong> {{ $t('tools.abstractAccount.featureMultiSigDesc') }}</li>
              <li><strong>{{ $t('tools.abstractAccount.featureDome') }}</strong> {{ $t('tools.abstractAccount.featureDomeDesc') }}</li>
              <li><strong>{{ $t('tools.abstractAccount.featureEip712') }}</strong> {{ $t('tools.abstractAccount.featureEip712Desc') }}</li>
            </ol>
          </div>
        </div>
            ...
            <div v-if="activeTab === 'create'" class="p-6 pt-0 flex-1 space-y-8" :class="{ 'opacity-50 pointer-events-none': !connectedAccount }">
              <div class="space-y-4">
                ...
                <div v-for="(admin, index) in admins" :key="'admin-'+index" class="flex items-center gap-2">
                  <input type="text" v-model="admins[index]" :aria-label="$t('tools.abstractAccount.adminAddressAria', { n: index + 1 })" class="form-input w-full bg-surface text-high font-mono text-sm rounded-xl shadow-inner focus:ring-2 focus:ring-indigo-500/20 hover:border-indigo-400 focus:border-indigo-400 transition-all outline-none" :placeholder="$t('tools.abstractAccount.addressInputPlaceholder')" />
                  <button @click="admins.splice(index, 1)" class="p-2 text-red-500 hover:bg-red-50 rounded-lg shrink-0" :aria-label="$t('tools.abstractAccount.removeAdminAria')">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                  </button>
                </div>
                <div class="flex items-center justify-between">
                  <button @click="admins.push('')" class="text-xs text-indigo-600 font-semibold hover:underline">{{ $t('tools.abstractAccount.addAdminButton') }}</button>
                  <div class="flex items-center gap-2">
                     <span class="text-xs font-bold text-high">{{ $t('tools.abstractAccount.thresholdLabel') }}</span>
                     <input type="number" v-model.number="adminThreshold" min="1" :max="Math.max(1, admins.length)" :aria-label="$t('tools.abstractAccount.adminThresholdAria')" class="form-input w-20 py-1 bg-surface text-high font-mono text-sm rounded-xl shadow-inner focus:ring-2 focus:ring-indigo-500/20 hover:border-indigo-400 focus:border-indigo-400 transition-all outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <h3 class="text-base font-bold text-high border-b border-line-soft pb-2 flex items-center gap-2">
                  ...
                  {{ $t('tools.abstractAccount.operationalManagers') }} <span class="font-normal text-mid ml-1 text-xs">{{ $t('tools.abstractAccount.optionalLabel') }}</span>
                </h3>
                <p class="text-xs text-mid">{{ $t('tools.abstractAccount.managersHint') }}</p>
                <div v-for="(manager, index) in managers" :key="'manager-'+index" class="flex items-center gap-2">
                  <input type="text" v-model="managers[index]" :aria-label="$t('tools.abstractAccount.managerAddressAria', { n: index + 1 })" class="form-input w-full bg-surface text-high font-mono text-sm rounded-xl shadow-inner focus:ring-2 focus:ring-indigo-500/20 hover:border-indigo-400 focus:border-indigo-400 transition-all outline-none" :placeholder="$t('tools.abstractAccount.addressInputPlaceholder')" />
                  <button @click="managers.splice(index, 1)" class="p-2 text-red-500 hover:bg-red-50 rounded-lg shrink-0" :aria-label="$t('tools.abstractAccount.removeManagerAria')">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                  </button>
                </div>
                <div class="flex items-center justify-between">
                  <button @click="managers.push('')" class="text-xs text-indigo-600 font-semibold hover:underline">{{ $t('tools.abstractAccount.addManagerButton') }}</button>
                  <div class="flex items-center gap-2">
                     <span class="text-xs font-bold text-high">{{ $t('tools.abstractAccount.thresholdLabel') }}</span>
                     <input type="number" v-model.number="managerThreshold" min="0" :max="Math.max(0, managers.length)" :aria-label="$t('tools.abstractAccount.managerThresholdAria')" class="form-input w-20 py-1 bg-surface text-high font-mono text-sm rounded-xl shadow-inner focus:ring-2 focus:ring-indigo-500/20 hover:border-indigo-400 focus:border-indigo-400 transition-all outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                <h3 class="text-base font-bold text-high border-b border-line-soft pb-2 flex items-center gap-2">
                  ...
                  {{ $t('tools.abstractAccount.domeRecoveryAccounts') }} <span class="font-normal text-mid ml-1 text-xs">{{ $t('tools.abstractAccount.optionalLabel') }}</span>
                </h3>
                <p class="text-xs text-mid">{{ $t('tools.abstractAccount.domesHint') }}</p>
                <div v-for="(dome, index) in domes" :key="'dome-'+index" class="flex items-center gap-2">
                  <input type="text" v-model="domes[index]" :aria-label="$t('tools.abstractAccount.domeAddressAria', { n: index + 1 })" class="form-input w-full bg-surface text-high font-mono text-sm rounded-xl shadow-inner focus:ring-2 focus:ring-indigo-500/20 hover:border-indigo-400 focus:border-indigo-400 transition-all outline-none" :placeholder="$t('tools.abstractAccount.addressInputPlaceholder')" />
                  <button @click="domes.splice(index, 1)" class="p-2 text-red-500 hover:bg-red-50 rounded-lg shrink-0" :aria-label="$t('tools.abstractAccount.removeDomeAria')">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                  </button>
                </div>
                <div class="flex items-center justify-between">
                  <button @click="domes.push('')" class="text-xs text-indigo-600 font-semibold hover:underline">{{ $t('tools.abstractAccount.addDomeButton') }}</button>
                  <div class="flex items-center gap-2">
                     <span class="text-xs font-bold text-high">{{ $t('tools.abstractAccount.thresholdLabel') }}</span>
                     <input type="number" v-model.number="domeThreshold" min="0" :max="Math.max(0, domes.length)" :aria-label="$t('tools.abstractAccount.domeThresholdAria')" class="form-input w-20 py-1 bg-surface text-high font-mono text-sm rounded-xl shadow-inner focus:ring-2 focus:ring-indigo-500/20 hover:border-indigo-400 focus:border-indigo-400 transition-all outline-none" />
                  </div>
                </div>
                
                <div class="flex items-center gap-4 mt-3" v-if="domes.length > 0">
                  <div class="flex-1">
                    <label for="dome-timeout" class="block text-xs font-bold text-high mb-1">{{ $t('tools.abstractAccount.inactivityTimeout') }}</label>
                    <input id="dome-timeout" type="number" v-model.number="domeTimeout" class="form-input w-full py-2 bg-surface text-high font-mono text-sm rounded-xl shadow-inner focus:ring-2 focus:ring-indigo-500/20 hover:border-indigo-400 focus:border-indigo-400 transition-all outline-none" :placeholder="$t('tools.abstractAccount.inactivityTimeoutPlaceholder')" />
                  </div>
                </div>
              </div>
              ...
              <div class="pt-6 border-t border-line-soft flex justify-end">
                 ...
                 >
                   ...
                   {{ isCreating ? $t('tools.abstractAccount.registeringButton') : $t('tools.abstractAccount.deploySmartAccountButton') }}
                 </button>
              </div>
            ...
            <div v-else-if="activeTab === 'source'" class="p-6 pt-0">
              ...
              <div class="relative group mt-2 rounded-xl overflow-hidden border border-line-soft">
                <div class="absolute right-3 top-3 z-10 flex gap-2">
                  <button @click="copyCode(sourceFiles[activeFileIdx].content)" class="p-2 bg-surface/80 backdrop-blur-sm border border-line-soft rounded-lg text-low hover:text-high transition-colors shadow-sm" :aria-label="$t('tools.abstractAccount.copyCodeAria')">
