      ...
      <div class="etherscan-card p-6">
        <div class="max-w-3xl mx-auto space-y-6">
          <div v-if="!connectedAccount" class="text-center py-10 border border-dashed border-line-soft rounded-xl bg-surface-muted">
             ...
             <p class="text-sm text-mid">Please connect your NeoLine or O3 wallet to deploy the Abstract Account contract.</p>
          </div>

          <template v-else>
            <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-900/50 rounded-xl p-4 text-sm text-blue-800 dark:text-blue-300">
              This tool automatically bundles our pre-compiled <strong>AbstractAccount.nef</strong> and deploys it on your behalf. You will be set as the initial admin unless you change the fields below.
            </div>

            <div class="space-y-4">
              <h2 class="text-lg font-bold text-high border-b border-line-soft pb-2">Admin Configuration</h2>
              <p class="text-sm text-mid">Admins have full control. They can upgrade the contract, modify thresholds, and manage whitelists/blacklists.</p>
              
              <div>
                <label class="block text-sm font-semibold text-high mb-2">Admin Addresses (One per line)</label>
                <textarea v-model="form.admins" rows="3" class="form-input w-full font-mono text-sm" placeholder="N..."></textarea>
              </div>

              <div>
                <label class="block text-sm font-semibold text-high mb-2">Admin Signature Threshold</label>
                <input type="number" v-model.number="form.adminThreshold" min="1" class="form-input w-full" />
              </div>
            </div>

            <div class="space-y-4 pt-4 border-t border-line-soft">
              <h2 class="text-lg font-bold text-high border-b border-line-soft pb-2">Manager Configuration</h2>
              <p class="text-sm text-mid">Managers can execute transactions (like token transfers) on behalf of the Abstract Account, subject to limits and whitelists.</p>
              
              <div>
                <label class="block text-sm font-semibold text-high mb-2">Manager Addresses (One per line)</label>
                <textarea v-model="form.managers" rows="3" class="form-input w-full font-mono text-sm" placeholder="N..."></textarea>
              </div>

              <div>
                <label class="block text-sm font-semibold text-high mb-2">Manager Signature Threshold</label>
                <input type="number" v-model.number="form.managerThreshold" min="0" class="form-input w-full" />
              </div>
            </div>
            ...
            <div class="pt-6 flex justify-end">
               <button 
                 ...
                 class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-6 py-2.5 text-sm font-bold text-white hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm active:scale-95"
               >
                 <svg v-if="isDeploying" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                 {{ isDeploying ? 'Deploying...' : 'Deploy Abstract Account' }}
               </button>
            </div>
