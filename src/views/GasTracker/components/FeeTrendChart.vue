<template>
  <div class="etherscan-card mb-6 p-5">
    <h2 class="text-high mb-1 text-base font-semibold">{{ $t("gasTracker.feeTrendTitle") }}</h2>
    <p class="text-low mb-4 text-xs">{{ $t("gasTracker.feeTrendSubtitle") }}</p>
    ...
    <div v-else class="text-low py-8 text-center text-sm">{{ $t("gasTracker.feeTrendEmpty") }}</div>
  </div>
</template>
