<template>
  <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="flex items-center gap-1">
      <button class="btn-page" :disabled="page <= 1" @click="goTo(1)">First</button>
      <button class="btn-page" :disabled="page <= 1" @click="goTo(page - 1)">&lt;</button>
      <span class="mx-2 text-sm text-gray-600 dark:text-gray-400">
        Page <strong>{{ page }}</strong> of <strong>{{ totalPages }}</strong>
      </span>
      <button class="btn-page" :disabled="page >= totalPages" @click="goTo(page + 1)">&gt;</button>
      <button class="btn-page" :disabled="page >= totalPages" @click="goTo(totalPages)">Last</button>
      <select
        v-if="showPageSize"
        :value="pageSize"
