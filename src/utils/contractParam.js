...
export function buildContractParam(type, rawValue) {
  ...
  // Hash160 RPC values must be hex (with or without 0x). Convert base58
  // N-addresses through normalizeHash160; if the input isn't a valid hex
  // hash or address, leave it untouched so test fixtures and partial
  // inputs round-trip unchanged.
  if (normalizedType === "Hash160" && value) {
