...
describe("AbstractAccountTool", () => {
  ...
  it("uses Global witness scope when deploying abstract account", async () => {
    ...
    expect(scope).toBe(128); // Global
  });

  it("sends deploy NEF as base64 ByteArray payload and Any null for args", async () => {
    const AbstractAccountTool = (await import("@/views/Tools/AbstractAccountTool.vue")).default;
    const wrapper = mount(AbstractAccountTool, {
      global: {
        stubs: {
    ...
    // Deployment state is null because the Unified AA Manager has zero initial config.
    expect(args[2]).toMatchObject({ type: "Any", value: null });
  });
  ...
  it("retries deploy with wallet-compat encoding after RPC_ERROR node return failure", async () => {
    ...
    expect(firstArgs[0].value).toMatch(/^[A-Za-z0-9+/]+={0,2}$/);
