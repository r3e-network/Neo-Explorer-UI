...
`;
  ...
  it("extracts backend allowlisted APIs from the current Go string slice format", () => {
    const source = `
package config
...
`;
    ...
    expect(extractBackendAllowedApis(source)).toEqual(
      new Set(["GetBlockCount", "GetTransactionList"])
    );
  });

  it("extracts backend allowlisted APIs from the legacy Go map format", () => {
    const source = `
package config

var Apis = map[string]bool{
  "GetBlockCount": true,
...
`;
  ...
  it("extracts backend allowlisted APIs from the current YAML realized-method list", () => {
    const source = `
proxy:
  uri:
...
`;
    ...
    expect(extractYamlRealizedMethods(source)).toEqual(
      new Set(["GetBlockCount", "GetTransactionList"])
    );
    expect(extractBackendAllowedApis(source)).toEqual(
      new Set(["GetBlockCount", "GetTransactionList"])
